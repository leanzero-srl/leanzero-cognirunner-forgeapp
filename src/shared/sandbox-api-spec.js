...
Forces the current issue into a status even when no normal transition path exists, by creating a temporary global transition, executing it, then removing it. The target status must already be part of the issue's workflow. \`opts.workflowName\` overrides the workflow (otherwise taken from the rule config). HEAVY: performs two workflow updates — use sparingly.`,
  ...
  {
    name: "moveToSprint",
    signature: "api.moveToSprint(sprintId)",
    returns: "{ success: true }",
    summary: "Moves the current issue into a sprint (Jira Software). Needs a scrum board + sprint id.",
    detail: "(sprintId) → { success }",
    example: "await api.moveToSprint(42);",
    promptDoc: "### api.moveToSprint(sprintId) → { success }\nAdds the current issue to the given sprint. The issue must be on a scrum board. Use api.moveToBacklog() to remove it.",
  },
  {
    name: "moveToBacklog",
    signature: "api.moveToBacklog()",
    returns: "{ success: true }",
    summary: "Moves the current issue to the backlog (removes it from its sprint).",
    detail: "() → { success }",
    example: "await api.moveToBacklog();",
    promptDoc: "### api.moveToBacklog() → { success }\nMoves the current issue out of any sprint and back to the backlog.",
  },
  {
    name: "rankIssue",
    signature: "api.rankIssue(relativeToKey, opts?)",
    returns: "{ success: true }",
    summary: "Ranks the current issue before (default) or after another issue on the board. opts.after=true to rank after.",
    detail: "(relativeToKey, opts?) → { success }",
    example: 'await api.rankIssue("PROJ-5");',
    promptDoc: "### api.rankIssue(relativeToKey, opts?) → { success }\nReorders the current issue relative to `relativeToKey` on the board. Default ranks BEFORE it; pass `{ after: true }` to rank after.",
  },
  {
    name: "editIssue",
  ...
  {
    name: "addLabels",
    signature: "api.addLabels(...labels)",
    returns: "{ success: true }",
    summary: "Adds labels to the current issue WITHOUT clobbering (uses update.add). Safe for concurrent post-functions.",
    detail: "(...labels) → { success }",
    example: 'await api.addLabels("triaged", "needs-review");',
    promptDoc: "### api.addLabels(...labels) → { success }\nAdds labels via `update.add` (merge, not replace). Use this instead of reading labels + api.updateIssue when other PFs may also be editing labels on the same transition.",
  },
  {
    name: "removeLabels",
    signature: "api.removeLabels(...labels)",
    returns: "{ success: true }",
    summary: "Removes labels from the current issue without clobbering (uses update.remove).",
    detail: "(...labels) → { success }",
    example: 'await api.removeLabels("wontfix");',
    promptDoc: "### api.removeLabels(...labels) → { success }\nRemoves labels via `update.remove` (merge, not replace).",
  },
  {
    name: "log",
