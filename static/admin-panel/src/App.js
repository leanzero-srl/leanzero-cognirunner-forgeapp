...
const injectStyles = () => {
  ...
  style.textContent = `
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .code-repo-row { display: flex; flex-direction: column; gap: 8px; padding: 10px 12px; border: 1px solid var(--border-color); border-radius: var(--r-md, 8px); }
    ...
    .code-hook-broken-text { font-size: 12px; font-weight: 600; line-height: 1.45; }
    .code-hook-broken-action { margin-top: 2px; padding: 6px 12px; border: 0; border-radius: var(--r-md, 8px); background: #fff; color: #0f172a; font-size: 12px; font-weight: 700; cursor: pointer; }
    .code-hook-broken-action:disabled { opacity: 0.7; cursor: default; }
    .code-pipe { display: flex; flex-direction: column; gap: 10px; padding: 12px; border: 1px solid var(--border-color); border-radius: var(--r-md, 8px); }
    .code-pipe-head { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; }
    .code-pipe-title { font-size: 13px; font-weight: 700; }
    .code-pipe-deploy { margin-left: auto; }
    ...
    .code-lock { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; font-size: 12px; }
    ...
    .code-run { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; font-size: 12px; }
    ...
    /* The resume card's destructive way out is the SECONDARY: an outline button, full
       ...
       card from pulling hardest towards throwing the draft away. */
    .va-resume-fresh { background: transparent; border: 1px solid var(--border-color); color: var(--text-color); font-weight: 600; }
    .va-resume-fresh:hover:not(:disabled) { border-color: #b45309; color: #b45309; }
    html[data-color-mode="dark"] .va-resume-fresh:hover:not(:disabled) { border-color: #f59e0b; color: #f59e0b; }

    /* A ticked desk with no queue: the widest intake the wizard can produce, stated.
       Solid ink at 600, no fill and no rail - it is a sentence about the answer above
       it, not an alarm about it. */
    .va-desk-whole { margin: 8px 0 0; font-size: 11.5px; font-weight: 600; line-height: 1.45; color: var(--text-color); }

    /* The people picker (mentions). The rows are a plain list of buttons; the picked
       people are the same solid amber chips every other pick on this surface wears. */
    .va-people { display: flex; flex-direction: column; gap: 8px; }
    .va-people-search { display: flex; align-items: center; gap: 8px; }
    .va-people-search .lst-input { flex: 1; }
    .va-people-results { display: flex; flex-direction: column; border: 1px solid var(--border-color); border-radius: var(--r-md, 8px); overflow: hidden; }
    ...
    .va-people-tail { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 11px; font-weight: 600; color: var(--text-secondary); }
    .va-people-row:hover:not(:disabled) .va-people-tail { color: inherit; }
    .va-people-already { margin-left: auto; font-size: 11px; font-weight: 700; color: var(--text-secondary); }
...
const injectCopiedComponentStyles = () => {
  ...
  style.textContent = `
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    /* F-129 — CANCELLED (tenant Stop-all) outcome for every queued-AI surface:
       FunctionBlock generate + Fix with AI, SkillEditor distill, ReviewPanel review.
       A cancel is an OPERATOR ACTION, not an AI failure, so it must never wear the red
       error treatment — neutral slate, solid fill, white text (no washed-out tint, no
       left accent rail). Dark variant is one shade lighter, per the hue map. */
    .async-cancelled-note {
      display: flex;
      align-items: flex-start;
      gap: 8px;
      margin: 8px 0;
    ...
    .async-cancelled-note .acn-text { flex: 1; }
    .async-cancelled-note .acn-dismiss {
      background: transparent;
      border: none;
      cursor: pointer;
      color: #ffffff;
    ...
    html[data-color-mode="dark"] .async-cancelled-note { background: #64748b; }

    /* F-133 — a generate that FAILED while the step already had code. The code, its
       provenance and its dry-run verdict were all KEPT (a failed regenerate never
       replaces the author's work, and there is no undo for generate). This IS a
       failure, so unlike the slate cancel note it wears solid saturated red with
       white text — no washed-out tint, no left accent rail. Dark is one shade lighter. */
    .async-error-note {
      display: flex;
      align-items: flex-start;
      gap: 8px;
    ...
    .async-error-note .aen-text { flex: 1; }
    .async-error-note .aen-retry {
      background: #ffffff;
      color: #dc2626;
      border: none;
      border-radius: 4px;
    ...
    /* F-155 — the merged badge cannot offer a veto, so it says where the row lives. */
    .memory-saved-note {
      font-size: 11px;
      font-weight: 600;
      color: var(--text-secondary);
...
function App() {
  ...
  return (
    <div className="container">
      ...
      {activeTab === "listeners" && (
        <ListenersTab invoke={invoke} isAdmin={isAdmin} userRole={userRole} siteUrl={siteUrl} router={router} roleUnknown={roleUnknown} accountId={accountId} />
      )}

      {/* Agents Tab — virtual administrators (1.5) */}
      {activeTab === "agents" && (
        <AgentsTab invoke={invoke} isAdmin={isAdmin} userRole={userRole} roleUnknown={roleUnknown} accountId={accountId} />
      )}

      {/* Scheduled Jobs Tab — cron rules */}
      {activeTab === "jobs" && (
        <JobsTab invoke={invoke} isAdmin={isAdmin} userRole={userRole} roleUnknown={roleUnknown} accountId={accountId} />
      )}
