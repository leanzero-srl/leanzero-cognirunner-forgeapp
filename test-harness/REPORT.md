...
- **✅ Forge conditions ARE enforced on the REST transition path (F3 — RE-DIAGNOSED 2026-08-12)** — the earlier "REST bypasses conditions" conclusion was wrong. The condition lambda was never invoked because `function:` is not a property of `jira:workflowCondition`; conditions passed everywhere because the manifest shipped `expression: "true"`. Both are fixed, and reg-conditions-enforce.mjs proves blocking on GET /transitions AND POST /transitions.
