...
/* THE `route` TAG (F-441). `@forge/api` refuses any product request whose path is not a
   route object (`requireSafeUrl` throws), so this suite uses the REAL tag from the
   installed SDK — imported by FILE PATH, because the offline loader maps the "@forge/api"
   specifier to a mock whose `route` returns a plain string and would hide exactly the
   defect under test.

   When node_modules is absent (a worktree, a checkout that has not installed), a stand-in
   with the same contract is used and the run SAYS SO: the assertions about OUR client still
   hold, but the SDK's own gate was not exercised in that run. A silent fallback would be a
   green tick for an unrun check. */
const sdkSafeUrl = path.join(here, "..", "..", "node_modules", "@forge", "api", "out", "safeUrl.js");
let sdk = null;
try {
  sdk = await import(pathToFileURL(sdkSafeUrl).href).then((m) => m.default || m);
} catch (_) {
  sdk = null;
}
const SDK_PRESENT = !!(sdk && typeof sdk.route === "function" && typeof sdk.requireSafeUrl === "function");
if (!SDK_PRESENT) {
  console.log("confluence-client: NOTE — @forge/api is not installed here; the route tag is a "
    + "stand-in and the SDK's own requireSafeUrl gate was NOT exercised in this run.");
  class StandInRoute { constructor(value) { this.value = value; } }
  const escape = (v, mode) => {
    if (mode === "path") {
      const str = String(v);
      if (/\.\.|[/\\?#]/.test(str)) throw new Error("Disallowing path manipulation attempt.");
      return str;
    }
  ...
  sdk = {
    route: (tpl, ...vals) => {
      ...
      for (let i = 0; i < tpl.length; i++) {
        if (/[?#]/.test(tpl[i])) mode = "query";
        out += tpl[i];
        if (i >= vals.length) break;
        out += escape(vals[i], mode);
      }
      return new StandInRoute(out);
    },
    isRoute: (x) => x instanceof StandInRoute,
    requireSafeUrl: (x) => {
      if (x instanceof StandInRoute) return x;
      throw new Error("You must create your route using the 'route' export from '@forge/api'.");
    },
  };
}
const { route: sdkRoute, isRoute, requireSafeUrl } = sdk;
const createClient = (deps = {}) => createConfluenceClient({ route: sdkRoute, ...deps });
...
function mock(handler) {
  ...
  const request = async (p, init) => {
    // A route object, never a string (F-441) — asserted explicitly below; unwrapped here
    // so every existing path assertion still reads the URL.
    const call = { path: isRoute(p) ? p.value : p, raw: p, method: (init && init.method) || "GET", init };
    calls.push(call);
 ...
 * ------------------------------------------------------------------ */
{
  ...
  const c = createClient({ request: t.request });
...
await expectErr(() => createClient({ request: mock(() => ({ status: 200, body: {} })).request })
...
await expectErr(() => createClient({ request: mock(() => ({ status: 404, body: "nope" })).request })
  .getPage({ id: "999" }), "not_found", "getPage on a missing page");
await expectErr(() => createClient({ request: mock(() => ({ status: 403, body: "forbidden" })).request })
  .getPage({ id: "9" }), "auth", "getPage without the scope");
await expectErr(() => createClient({ request: mock(() => ({ status: 500, body: "boom" })).request })
...
await expectErr(() => createClient({ request: mock(() => ({ status: 200, body: "<!doctype html><html>hi</html>" })).request })
...
await expectErr(() => createClient({ request: mock(() => ({ status: 200, body: { results: [] } })).request })
...
await expectErr(() => createClient({ request: mock(() => ({ status: 200, body: { results: [{ id: "1" }] } })).request })
...
}
{
  ...
  const c = createClient({ request: t.request, sleep: async () => {} });
  const p = await c.getPage({ id: "1" });
 ...
 * ------------------------------------------------------------------ */
{
  ...
  const c = createClient({ request: t.request, timeoutMs: 25 });
 ...
 * ------------------------------------------------------------------ */
{
  ...
  try {
    ...
    const request = async (raw) => {
      const path = isRoute(raw) ? raw.value : raw;
      calls.push(path);
    ...
    const c = createClient({ request, sleep: async () => {} });
...
}
{
  ...
  const r = await createClient({ request: t.request }).probeInstalled();
...
/* ------------------------------------------------------------------ *
 * THE PATH IS A `route`, NEVER A STRING (F-441)
 *
 * `@forge/api` wraps every product request in `requireSafeUrl(path)`, which THROWS unless
 * it is handed a route built by the `route` tag. This module passed plain template strings,
 * so every Confluence call failed in-app and surfaced as `code:"network"` — the header's
 * claim of a "plain-path call idiom" elsewhere in the repo was wrong: those sites pass
 * route VALUES through a variable.
 * ------------------------------------------------------------------ */
{
  // Positive control FIRST: prove the SDK gate actually refuses a string, on this very
  // version of @forge/api. A gate that has stopped refusing looks exactly like a pass.
  checks++;
  assert.throws(() => requireSafeUrl("/wiki/api/v2/pages/5"), /must create your route/,
    "the SDK refuses a plain string path");
  ok(isRoute(sdkRoute`/wiki/api/v2/pages/5`), "…and accepts what the route tag builds");
}
{
  const t = mock((call, n) => (n === 1
    ? { status: 200, body: { results: [{ id: 77 }] } }
    : { status: 200, body: { results: [{ id: "5", title: "T", version: { number: 1 }, body: { storage: { value: "<p>x</p>" } } }] } }));
  const c = createClient({ request: t.request, sleep: async () => {} });
  await c.probeInstalled();
  await c.searchCql({ cql: "type=page AND title~'a b'" });
  await c.getPage({ id: "5" });
  await c.getPageByTitle({ spaceKey: "DOCS", title: "Release notes" });
  ...
  for (const call of t.calls) {
    ...
    checks++;
    assert.doesNotThrow(() => requireSafeUrl(call.raw), `the SDK accepts ${call.path}`);
  }
  ok(t.calls[0].path === INSTALL_PROBE_PATH, "the probe route is the catalogued path");
}
{
  // ESCAPING HAPPENS ONCE. A title with a space and an ampersand is percent-encoded by the
  // tag — not by this module and then again by the tag, which is the double-encoded path
  // that removing `encodeURIComponent` from here prevents.
  const t = mock((call, n) => (n === 1
    ? { status: 200, body: { results: [{ id: 77 }] } }
    : { status: 200, body: { results: [] } }));
  const c = createClient({ request: t.request });
  await c.getPageByTitle({ spaceKey: "DOCS", title: "Q&A notes" });
  const q = t.calls[1].path;
  ok(q.includes("title=Q%26A%20notes"), `a query value is escaped exactly once (${q})`);
  ok(!q.includes("%2520") && !q.includes("%2526"), "and never twice");
}
{
  // A path SEGMENT that tries traversal is refused by the tag itself — the protection the
  // string path skipped entirely.
  const t = mock(() => ({ status: 200, body: {} }));
  const c = createClient({ request: t.request });
  checks++;
  await assert.rejects(() => c.getPage({ id: "../../admin" }), /path manipulation/,
    "a traversal attempt in a path segment is refused by route, before any request");
  ok(t.calls.length === 0, "and nothing was sent");
}
