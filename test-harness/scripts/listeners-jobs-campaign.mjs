...
async function cleanup() {
 ...
 for(const kind of ['listeners','jobs']){const list=must(await rulesApi[kind].list(),'final collection')[kind];assert.ok(!list.some(x=>state[kind].some(y=>y.id===x.id)));for(const old of state.baseline[kind]){const current=list.find(x=>x.id===old.id);assert.ok(current);for(const [key,value]of Object.entries(old))if(key!=='stats')assert.deepEqual(current[key],value,`${old.id} preserved ${key}`);}record(kind+' cleanup collection and unrelated state preserved');}
