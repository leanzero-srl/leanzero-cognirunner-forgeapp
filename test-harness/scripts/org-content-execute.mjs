...
async function execute(mode,site,key,limit){
  ...
  try{
    ...
    const walked=await runPopulationRows(limit?population.rows.slice(0,limit):population.rows,async row=>{
      ...
      if(!match){
        demand(!receipt||(receipt.stage==='rejected-create'&&[400,403,404,422].includes(receipt.rejection?.status)),'Pending issue creation is not visible; never repeat an ambiguous POST');
      ...
      }else{demand(receipt,'Matching issue lacks campaign creation receipt; preserve it');demand(receipt.stage!=='rejected-create','A rejected create has a matching issue; reconcile identity before adoption');}
